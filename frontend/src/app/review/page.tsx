                  ...
                  <div className="space-y-3">
                    {alerts.map((alert, index) => (
                      <article key={`${alert.timestamp}-${index}`} className="rounded-[1.2rem] border border-black/12 bg-white/90 p-3 shadow-[0_10px_20px_rgba(0,0,0,0.08)] transition duration-300 hover:-translate-y-0.5">
                        ...
                        {alert.bounding_box && (
                          <div className="mt-2 inline-flex items-center rounded-full border border-black/12 bg-black/[0.03] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-black/65">
                            Marked block: {alert.bounding_box.join(", ")}
                          </div>
                        )}
